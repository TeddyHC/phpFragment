...
class MyDateTime
{
    ...
    {
        return MyDateTime::valueOf(date($format));
    }
    ...
    {
        return new MyDateTime(date($format, time() - self::SECOND_OF_DAY));
    }

    public static function dayBeforeYesterday($format = self::DEFAULT_DATE_FORMAT)
    {
        return new MyDateTime(date($format, time() - self::SECOND_OF_DAY * 2));
    }
    ...
    {
        return new MyDateTime(date(self::DEFAULT_XDATETIME_FORMAT, time()+(int)$days * self::SECOND_OF_DAY));
    }
    ...
    {
        return $this->equals(MyDateTime::valueOf(self::ZERO));
    }

    public static function getZero()
    {
        return MyDateTime::valueOf(self::ZERO);
    }
    ...
    public static function printTime4Redesign($time)
    {
        ...
        $tmpTime = MyDateTime::valueOfTime($time)->toStringByFormat(self::DEFAULT_DATE_FORMAT);
        if($alltime < 60)
        {
            return '刚刚';
        }
        ...
        {
            return '今天'.MyDateTime::valueOfTime($time)->toStringByFormat("H:i");
        }
        else if(strtotime($tmpTime) == strtotime(self::yesterday()))
        {
            return '昨天'.MyDateTime::valueOfTime($time)->toStringByFormat("H:i");
        }
        else if(self::yearDiff(MyDateTime::valueOfTime($time), MyDateTime::now())) //当前年份大于提交年份
        {
            return MyDateTime::valueOfTime($time)->toStringByFormat("Y.m.d H:i");
        }
        else
        {
            return MyDateTime::valueOfTime($time)->toStringByFormat("m.d H:i");
        }
    }

    public static function dayAfterTomorrow($format = self::DEFAULT_DATE_FORMAT)
    {
        return new MyDateTime(date($format, time() + 3600 * 24 * 2));
    }
    ...
    public static function printTime4Touch($time, $format="Y-m-d H:i:s")
    {
        ...
        if ($alltime < 60) {
            ...
            return MyDateTime::valueOfTime($time)->toStringByFormat($format);
        }
    }

    public static function getDateDiffDesc(MyDateTime $fromTime, MyDateTime $endTime)
    {
        $diff = MyDateTime::valueOf($fromTime)->getDateDiff($endTime);
        $yearDiff = $diff['year'];
    ...
    {
        return ($this->getTime() == MyDateTime::valueOf(self::DEFAULT_START_TIME)->getTime());
    }
    ...
    {
        return MyDateTime::valueOf(self::FAKE_INFINITYDATE);
    }
    ...
    public static function birthday2Age($birthday)
    {
        ...
        try
        {
            $diff = MyDateTime::valueOf($birthday)->getDateDiff(MyDateTime::now());
        }
        catch (Exception $ex)
        {
            return '';
        ...
        else
        {
            ...
            {
                $birthday = MyDateTime::valueOf($birthday);
                if(MyDateTime::now()->getMonth() != $birthday->getMonth())
                {
                    $age = "1个月";
                }
            }

...
class XString
{
    ...
    public static function splitStringToArray($str, array $delimiterArr = [])
    {
        mb_regex_encoding('UTF-8');
        $delimiters = '\r\n\s\t,;`，；、\/';
        if (!empty($delimiterArr)) {
            $delimiters = implode('', $delimiterArr);
        }
        $pattern = "[{$delimiters}]";

        $strArr = mb_split($pattern, $str);
    ...
    private static function convertToEncoding($str, $toEncoding)
    {
        ...
        } elseif ($maybechset != $toEncoding) { // 不是 GBK，转换一下
    ...
    {
        return is_array($value) ? array_map('XString::toGbkDeep', $value) : (self::convertToGbk($value));
    }
    ...
    {
        $content = self::convertToUnicode($content);
        ...
        $content = preg_replace('/((\n )|(\n))+/', "\n", $content);
        $content = self::convertToGbk($content);
    ...
    public static function getLengthOfGBKString($str)
    {
        ...
        return mb_strlen(self::cntrim($str));
    }
